...
def run(params):
    ...
    def build_model(embedding_matrix, max_sentence_length, number_of_classes):
        ...
        dropout_1 = Dropout(0.1)(embedded_sequences)
        l_lstm = Bidirectional(LSTM(200, return_sequences=True))(dropout_1)
        dropout_2 = Dropout(0.3)(l_lstm)
        preds = TimeDistributed(Dense(number_of_classes + 1, activation='softmax'))(dropout_2)
    ...
    class F1ScoreModelCheckpointer(Callback):
        ...
        def on_epoch_end(self, batch, logs={}):
            ...
            f1_dev = f1_score(test_truth, test_pred, list(index_to_label.values()), average='macro')

            if f1_dev > self.best_f1 :
                print("Epoch {0}: F1 scores improved from {1} to {2}, saving model to results\lstm.model".format(batch, self.best_f1,f1_dev))
                model.save(filepath=model_path, overwrite=True)
                self.best_f1 = f1_dev
            else:
                print("Epoch {0}: F1 scores did not improved from {1} to {2}".format(batch, f1_dev,self.best_f1))
